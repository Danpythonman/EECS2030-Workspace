...
public class StarterTests {
	...
	public void test_cumulative_01() {
		...
		op =  new Projection(seq2, seq1);
		assertEquals("Projecting [2, 1, 6, 3, 1, 4, 5, 3] to [] results in: []", op.toString());

		op =  new Projection(seq1, seq1);
		assertEquals("Projecting [] to [] results in: []", op.toString());

		op =  new Projection(seq2, seq2);
		assertEquals("Projecting [2, 1, 6, 3, 1, 4, 5, 3] to [2, 1, 6, 3, 1, 4, 5, 3] results in: [2, 1, 6, 3, 1, 4, 5, 3]", op.toString());
	}

	@Test
	public void test_cumulative_02() {
		/*
		 * You may want to also test:
		 * 	1) An empty seq1 [] occurs within any seq2 (either empty or non-empty).
		 * 	2) A non-empty seq1 does not occur within an empty seq2 []. 
		...
		op = new OccursWithin(seq2, seq1);
		assertEquals("[2, 1, 6, 3, 1, 4, 5, 3] does not occur within []", op.toString());

		op = new OccursWithin(seq1, seq1);
		assertEquals("[] occurs within []", op.toString());

		op = new OccursWithin(seq2, seq2);
		assertEquals("[2, 1, 6, 3, 1, 4, 5, 3] occurs within [2, 1, 6, 3, 1, 4, 5, 3]", op.toString());

		int[] seq3 = {1, 2, 3, 4, 5};
		int[] seq4 = {1, 2, 3};
		op = new OccursWithin(seq3, seq4);
		assertEquals("[1, 2, 3, 4, 5] does not occur within [1, 2, 3]", op.toString());
	}

	@Test
	public void test_cumulative_03() {
		/*
		 * You may want to also test the case where an empty sequence has only one prefix: []  
		 */
	...
	@Test
	public void test_cumulative_04() {
		/*
		 * You may also want to test cases where:
		 * 	- The concatenation involves more added operations.
		 * 	- Some added operations at the beginning or in the middle of the list may result in empty sequences.  
		...
		try {
			...
			assertEquals("Concat([], [], []) = []", evaluator.toString());
		}
		catch(IllegalOperationException e) {
			fail();
		}
		...
		try {
			...
			assertEquals("Concat([1, 3, 1, 5, 3], [1, 3, 1, 5, 3], [1, 3, 1, 5, 3], [], [], [0, 1, 4, 9]) = [1, 3, 1, 5, 3, 1, 3, 1, 5, 3, 1, 3, 1, 5, 3, 0, 1, 4, 9]", evaluator.toString());
		}
		catch(IllegalOperationException e) {
			fail();
		}
		...
		try {
			...
			assertEquals("Concat cannot be evaluated due to 5 incompatile operations.", evaluator.toString());
		}
		catch(IllegalOperationException e) {
			fail();
		}
	...
	@Test
	public void test_cumulative_05() {
		/*
		 * You may also want to test cases where:
		 * 	- The filter evaluator stores more added operations.
		 * 	- Some added operations at the beginning or end of the list may result in false (which will be filtered out).  
		...
		try {
			...
			assertEquals("Filter result is: _, true, _, true, _", evaluator.toString());
		}
		catch(IllegalOperationException e) {
			fail();
		}
		...
		try {
			...
			assertEquals("Filter result is: true", evaluator.toString());
		}
		catch(IllegalOperationException e) {
			fail();
		}
		...
		try {
			...
			assertEquals("Filter result is: _", evaluator.toString());
		}
		catch(IllegalOperationException e) {
			fail();
		}
		...
		try {
			...
			evaluator.addOperation("op:projection", seq2, seq4);
			evaluator.addOperation("op:projection", seq2, seq4);
			evaluator.addOperation("op:projection", seq2, seq4);
			evaluator.addOperation("op:sumsOfPrefixes", seq2, seq4);
			evaluator.addOperation("op:sumsOfPrefixes", seq2, seq4);

			evaluator.addOperation("op:occursWithin", seq3, seq4);

			assertEquals("Filter cannot be evaluated due to 5 incompatile operations.", evaluator.toString());
		}
		catch(IllegalOperationException e) {
			fail();
		}
