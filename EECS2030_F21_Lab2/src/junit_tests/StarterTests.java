...
public class StarterTests {
	...
	public void test_cumulative_01() {
		...
		HealthRecord tom = new HealthRecord("Tom", 5);
		HealthRecord jim = new HealthRecord("Jim", 5);
		HealthRecord mark = new HealthRecord("Mark", 10);

		try {
			for (int i = 0; i < 10; i++) {
				vs.addDistribution(v3, 1); /* distribution of AZ */
			}
			...
			fail("InsufficientVaccineDosesException was expected to be thrown, but it was not thrown");
		} catch (UnrecognizedVaccineCodeNameException e) {
			fail("Unexpected UnrecognizedVaccineCodeNameException thrown");
		} catch (TooMuchDistributionException e) {
			fail("Unexpected TooMuchDistributionException thrown");
		} catch (InsufficientVaccineDosesException e) {
			vs.administer("October 10, 2021");

			assertEquals(alan.getVaccinationReceipt(), "Alan has not yet received any doses.");
			assertEquals(tom.getVaccinationReceipt(), "Tom has not yet received any doses.");
			assertEquals(jim.getVaccinationReceipt(), "Jim has not yet received any doses.");
			assertEquals(
				"Number of doses Mark has received: 10 ["
				+ "Recognized vaccine: AZD1222 (Non Replicating Viral Vector; Oxford/AstraZeneca) in North York General Hospital on October 10, 2021; "
				+ "Recognized vaccine: AZD1222 (Non Replicating Viral Vector; Oxford/AstraZeneca) in North York General Hospital on October 10, 2021; "
				+ "Recognized vaccine: AZD1222 (Non Replicating Viral Vector; Oxford/AstraZeneca) in North York General Hospital on October 10, 2021; "
				+ "Recognized vaccine: AZD1222 (Non Replicating Viral Vector; Oxford/AstraZeneca) in North York General Hospital on October 10, 2021; "
				+ "Recognized vaccine: AZD1222 (Non Replicating Viral Vector; Oxford/AstraZeneca) in North York General Hospital on October 10, 2021; "
		...
		try {
			vs.addDistribution(v3, 1); /* 1st distribution of AZ */
			vs.addDistribution(v1, 1); /* 1st distribution of Moderna */
			vs.addDistribution(v2, 2); /* 1st distribution of Pfizer (2 doses) */
			vs.addDistribution(v1, 1); /* distribution of Moderna */
			vs.addDistribution(v1, 1); /* distribution of Moderna */
			vs.addDistribution(v2, 2); /* distribution of Pfizer (2 doses) */
			...
			assertEquals(
				"Number of doses Alan has received: 1 ["
				+ "Recognized vaccine: AZD1222 (Non Replicating Viral Vector; Oxford/AstraZeneca) in North York General Hospital on October 10, 2021"
				+ "]",
				alan.getVaccinationReceipt()
			);
			assertEquals(
				"Number of doses Jim has received: 3 ["
				+ "Recognized vaccine: mRNA-1273 (RNA; Moderna) in North York General Hospital on October 10, 2021; "
				+ "Recognized vaccine: mRNA-1273 (RNA; Moderna) in North York General Hospital on October 10, 2021; "
				+ "Recognized vaccine: mRNA-1273 (RNA; Moderna) in North York General Hospital on October 10, 2021"
				+ "]",
				jim.getVaccinationReceipt()
			);
			assertEquals(
				"Number of doses Tom has received: 2 ["
				+ "Recognized vaccine: BNT162b2 (RNA; Pfizer/BioNTech) in North York General Hospital on October 10, 2021; "
				+ "Recognized vaccine: BNT162b2 (RNA; Pfizer/BioNTech) in North York General Hospital on October 10, 2021"
				+ "]",
				tom.getVaccinationReceipt()
			);

			assertEquals(2, vs.getNumberOfAvailableDoses());

			assertEquals(0, vs.getNumberOfAvailableDoses("AZD1222"));
			assertEquals(0, vs.getNumberOfAvailableDoses("mRNA-1273"));
			assertEquals(2, vs.getNumberOfAvailableDoses("BNT162b2"));

			assertEquals("North York General Hospital has 2 available doses: <0 doses of Oxford/AstraZeneca, 0 doses of Moderna, 2 doses of Pfizer/BioNTech>", vs.toString());
		} catch (UnrecognizedVaccineCodeNameException e) {
			fail("Unexpected UnrecognizedVaccineCodeNameException thrown");
		} catch (TooMuchDistributionException e) {
			fail("Unexpected TooMuchDistributionException thrown");
		} catch (InsufficientVaccineDosesException e) {
			fail("Unexpected InsufficientVaccineDosesException thrown");
		}

		VaccinationSite vs2 = new VaccinationSite("Toronto General Hospital", 10000);

		try {
			vs2.addDistribution(v2, 10000);
			assertEquals(10000, vs2.getNumberOfAvailableDoses());

			HealthRecord bob = new HealthRecord("Bob", 5);

			vs2.bookAppointment(bob);

			vs2.administer("October 10, 2021");

			assertEquals(9999, vs2.getNumberOfAvailableDoses());
		} catch (UnrecognizedVaccineCodeNameException e) {
			fail("Unexpected UnrecognizedVaccineCodeNameException thrown");
		} catch (TooMuchDistributionException e) {
			fail("Unexpected TooMuchDistributionException thrown");
		...
		try {
			vs2.addDistribution(new Vaccine("NOT-APPROVED", "not approved vaccine type", "Not Recognized by Canada"), 1);
			fail("Expected UnrecognizedVaccineCodeNameException to be thrown, but was not thrown");
		} catch (UnrecognizedVaccineCodeNameException e) {
			// Expected
		} catch (TooMuchDistributionException e) {
			fail("Unexpected TooMuchDistributionException thrown");
		...
		try {
			vs2.addDistribution(v2, 1);
			fail("Expected TooMuchDistributionException to be thrown, but was not");
		} catch (UnrecognizedVaccineCodeNameException e) {
			fail("Unexpected UnrecognizedVaccineCodeNameException thrown");
		} catch (TooMuchDistributionException e) {
			// Expected
